...
import { LAYOUT, textWidth, placeLegend, type Box, type Point } from '../subs/layout/src/index.js';
...
export function layoutLegend(
  ...
  const placed = placeLegend({ items: legendGroups.map(group => ({ key: group.id,
    width: groupWidth(group.title, group.entries),
    height: LAYOUT.legend.titleHeight + group.entries.length * LAYOUT.legend.rowHeight,
  })), maxWidth: width }, { gapX: LAYOUT.legend.columnGap, gapY: 12, padding: 0, orientation: 'horizontal' });

  for (const group of legendGroups) {
    const placement = placed.items.find(item => item.key === group.id);
    if (placement === undefined) throw new Error('Missing legend placement.');
    const cursorX = left + placement.box.x;
    const cursorY = bandTop + placement.box.y;
    const boxWidth = placement.box.width;
    const height = placement.box.height;
  ...
  const notesTop = bandTop + placed.bounds.y + placed.bounds.height + 16;
