 ...
 */
export const example1Declaration: ModelInput = defineDiagramModel({
  name: 'app',
  ...
  relays: [{ name: 'computeTotal', child: 'globalLibrary', destinations: ['descendants'] }],
  children: [
    {
      name: 'globalLibrary',
      relays: [{ name: 'computeTotal', child: 'moneyUtils', destinations: ['parent'] }],
      children: [{ name: 'moneyUtils', symbols: [{ name: 'computeTotal', destinations: ['parent'] }] }],
    },
    {
      name: 'invoicing',
      // The same mechanism as `app`'s, one level down: a "domain library" and a
      // "global library" differ only in altitude.
      relays: [{ name: 'InvoiceModel', child: 'invoicingLibrary', destinations: ['descendants'] }],
      children: [
        { name: 'invoicingLibrary', symbols: [{ name: 'InvoiceModel', destinations: ['parent'] }] },
        { name: 'invoiceComputation' },
        { name: 'invoicePDF' },
      ],
    },
    ...
    {
      name: 'shipping',
      symbols: [{ name: 'ShipmentPlan', destinations: ['descendants'] }],
      children: [
        { name: 'routingOptimization', symbols: [{ name: 'optimizeRoute', destinations: ['parent'] }] },
      ],
    },
  ],
});
