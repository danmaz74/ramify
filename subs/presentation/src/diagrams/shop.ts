...
import type { ModelInput, ModuleId, Model } from '../model-access.js';
 ...
 * ```
 */
export const shopDeclaration: ModelInput = defineDiagramModel({
  name: 'shop',
  symbols: [
    { name: 'Money', destinations: ['descendants'] },
    { name: 'formatDate', destinations: ['descendants'] },
  ],
  ...
  relays: [{ name: 'ProductId', child: 'catalog', destinations: ['descendants'] }],
  children: [
    {
      name: 'catalog',
      symbols: [
        { name: 'ProductId', destinations: ['parent'] },
        { name: 'SkuRules', destinations: ['descendants'] },
      ],
      // `reserveStock` becomes available in `catalog` and stops there:
      // `catalog` exposes it no further.
      children: [
        { name: 'search' },
        { name: 'inventory', symbols: [{ name: 'reserveStock', destinations: ['parent'] }] },
      ],
    },
    {
      name: 'checkout',
      ...
      relays: [{ name: 'PaymentApi', child: 'payment', destinations: ['descendants'] }],
      children: [
        { name: 'cart', symbols: [{ name: 'CartApi', destinations: ['parent'] }] },
        {
          name: 'payment',
          symbols: [
            { name: 'PaymentApi', destinations: ['parent'] },
...
export const shopTree: Model = context.tree;
