 ...
 */
export const example3Declaration: ModelInput = defineDiagramModel({
  name: 'app',
  ...
  relays: [
    { name: 'OrderService', child: 'orders', destinations: ['descendants'] },
    { name: 'resetOrderStore', child: 'orders', destinations: ['descendants'] },
  ],
  children: [
    {
      name: 'orders',
      // Two identical exposures. The tag on the second is the entire
      // difference between them, and test support is curated symbol by name:
      // tests never receive blanket private access.
      symbols: [
        { name: 'OrderService', destinations: ['parent'] },
        { name: 'resetOrderStore', destinations: ['parent'], tags: ['testing'] },
      ],
    },
    { name: 'billing' },
    { name: 'integration-tests', tags: ['testing'] },
  ],
});
