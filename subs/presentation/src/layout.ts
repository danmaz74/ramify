...
import { LAYOUT, textWidth, measureBounds, type Box, type Point } from '../subs/layout/src/index.js';
...
/** Presentation supplies the measured extents of the elements it renders. */
function include(boxes: Box[], x: number, y: number): void {
...
export function buildDiagramLayout(definition: DiagramDefinition = shopDiagram): DiagramLayout {
  ...
  const measured = measureBounds(bounds);
  const pad = LAYOUT.margin;
  const viewBox = {
    x: Math.floor(measured.x - pad),
    y: Math.floor(measured.y - pad),
    width: Math.ceil(measured.width + 2 * pad),
    height: Math.ceil(measured.height + 2 * pad),
  };
