...
import { buildDiagramModel } from '../model-access.js';
import { chainText, focusContext } from '../tree-diagram.js';

beforeAll(async () => {
  await access(new URL('../../../../site/static/diagrams/shop-focus-payment.svg', import.meta.url));
});

describe('the focus context', () => {
  const context = focusContext(buildDiagramModel(shopTreeDeclaration), 'payment');
