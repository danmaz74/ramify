...
import { LAYOUT, rowLabelDx, type Point } from '../../subs/layout/src/index.js';
import { buildDiagramLayout } from '../layout.js';
import { descendantsOf } from '../layout-nodes.js';
import { mayImport, moduleView } from '../model-access.js';
...
describe('node boxes', () => {
  it('gives every declared module a box', () => {
...
describe('propagation lanes', () => {
  ...
  it('assigns exposure directions to opposite sides of the edge', () => {
  ...
  it('lands one arrowhead on the parent for every exposure to the parent, and nothing further', () => {
    for (const decision of layout.propagation.decisions.filter((entry) => entry.kind === 'to-parent')) {
      ...
      expect(lanes[0]!.reaches).toBe(moduleView(shopTree, decision.decider)?.parent);
    }
  ...
  it('routes shortest span nearest the tree, one row each, below every node box', () => {
    ...
    for (const chord of rows) {
      ...
      for (const point of points(chord.d)) {
        // A chord can connect to an owner on a higher level. Cubic paths
        // previously exposed only their initial M point to this helper; sampled
        // curves expose the complete arc, whose upper bound includes both ends.
        expect(point.y).toBeGreaterThanOrEqual(Math.min(importer.box.y, owner.box.y));
      }
    }
