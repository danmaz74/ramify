...
beforeAll(async () => {
  // Missing emitter artifacts must fail, rather than become new snapshots.
  await Promise.all(['model-core', 'example1', 'example1a', 'example1b', 'example2', 'example3', 'example4']
    .map((name) => access(new URL(`../../../../site/static/diagrams/${name}.svg`, import.meta.url))));
});
