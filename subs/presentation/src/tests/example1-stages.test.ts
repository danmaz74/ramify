...
import { mayImport, buildDiagramModel, moduleView, type ModelInput } from '../model-access.js';
...
const subtreeOf = (declaration: ModelInput, name: string) => {
  const module = declaration.modules.find((item) => item.name === name)!;
  const includes = (id: string) => id === module.id || id.startsWith(`${module.id}/`);
  return { modules: declaration.modules.filter((item) => includes(item.id)),
    originals: declaration.originals.filter((item) => includes(item.id.owner)),
    exposures: declaration.exposures.filter((item) => includes(item.module)) };
};
...
describe('the build-up stages are regions of the full example 1', () => {
  it('stage A: the shipping subtree is the full diagram’s, under an empty root', () => {
    ...
    const root = moduleView(buildDiagramModel(example1aDiagram.declaration), 'app');
  ...
  it('stage B: the invoicing subtree is the full diagram’s, under a root that decides nothing', () => {
    ...
    const root = moduleView(buildDiagramModel(example1bDiagram.declaration), 'app');
    expect(root.owns ?? []).toEqual([]);
    expect(root.reExposes ?? []).toEqual([]);
    expect(root.children).toEqual(['invoicing']);
  });
