...
import { LAYOUT, polyline, placeLanes, wrapText, type Point } from '../subs/layout/src/index.js';
import {
  bottomCenter,
  placementGraph,
  descendantsOf,
...
export function allDiagramSymbols(context: DiagramContext): DiagramSymbol[] {
  return context.moduleOrder.flatMap((id) =>
    moduleView(context.tree, id).owns.map((owned) => ({ owner: id, name: owned.symbol })),
  );
}

function isAncestorOrSelf(tree: Model, ancestor: ModuleId, node: ModuleId): boolean {
  if (ancestor === node) {
    return true;
  }
...
export function enumerateDecisions(context: DiagramContext): PropagationDecision[] {
  ...
  const refs = allDiagramSymbols(context);
  const decisions: PropagationDecision[] = [];
  let order = 0;

  const emit = (kind: 'to-descendants' | 'to-parent', decider: ModuleId, carried: readonly DiagramSymbol[]): void => {
    if (carried.length === 0) {
      return;
    }
    const bundles = new Map<string, DiagramSymbol[]>();
    ...
    for (const [layer, bundle] of bundles) {
      const first = bundle[0] as DiagramSymbol;
      decisions.push({
...
function treeEdges(context: DiagramContext): EdgeKey[] {
  return context.moduleOrder.flatMap((parent) =>
    moduleView(context.tree, parent).children.map((child) => ({ parent, child })),
  );
}
...
function laneOffsets(
  ...
  descendantExposures.forEach((decision, index) => {
    offsets.set(decision.id, index + 1);
  });
...
export function layoutPropagation(
  ...
  const placed = placeLanes({ ...placementGraph(context, geometry), edges: edges.flatMap(edge => [
    { key: `${edge.parent}->${edge.child}`, from: edge.parent, to: edge.child, lane: 0, order: 0 },
    ...[...(offsetsByEdge.get(`${edge.parent}->${edge.child}`) ?? [])].map(([decision, lane], order) => ({
      key: `${edge.parent}->${edge.child}:${decision}`, from: edge.parent, to: edge.child, lane, order,
    })),
  ...
  const paths = new Map(placed.edges.map(edge => [edge.key, edge.points.map(point => ({ x: point.x, y: point.y + shiftY }))]));
  for (const edge of edges) {
    const key = `${edge.parent}->${edge.child}`;
    const base = paths.get(key)?.[0];
    const offsets = offsetsByEdge.get(key);
    if (base === undefined || offsets === undefined) throw new Error('Missing lane placement.');
    for (const decision of offsets.keys()) {
      offsets.set(decision, (paths.get(`${key}:${decision}`)?.[0]?.x ?? base.x) - base.x);
    }
  }
  ...
  const edgeLayouts: TreeEdgeLayout[] = edges.map((edge) => {
    ...
    return {
      ...
      parent: edge.parent,
      child: edge.child,
      d: polyline(paths.get(`${edge.parent}->${edge.child}`) ?? []),
      busY,
    };
  });
  ...
  for (const edge of edges) {
    ...
    for (const decision of decisions) {
      ...
      const points = paths.get(`${edge.parent}->${edge.child}:${decision.id}`) ?? [];
  ...
  const dots: DecisionDot[] = decisions.map((decision) => {
    ...
    if (decision.kind === 'to-descendants') {
      const firstChild = moduleView(tree, decision.decider).children[0];
    ...
    const parent = moduleView(tree, decision.decider).parent;
    if (parent === null) {
  ...
  for (const decision of decisions) {
    if (decision.kind === 'to-parent') {
      ...
      const parent = moduleView(tree, decision.decider).parent as ModuleId;
    ...
    const branchPoints = [decision.decider, ...descendantsOf(tree, decision.decider)].filter(
      (id) => moduleView(tree, id).children.length >= 2,
    );
    for (const branch of branchPoints) {
      const node = nodeOf(branch);
      const childEdges = moduleView(tree, branch).children.map((child) => `${branch}->${child}`);
      const firstChild = moduleView(tree, branch).children[0] as ModuleId;
