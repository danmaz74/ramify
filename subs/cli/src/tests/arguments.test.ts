...
describe('lightweight CLI arguments', () => {
  ...
  it.each(['status', 'stop'])('rejects daemon %s without substituting a batch run while its provider is absent', async action => {
    let calls = 0;
    ...
    expect([exit, calls, stdout.length, stderr]).toEqual([2, 0, 1, []]);
    expect(JSON.parse(stdout[0])).toMatchObject({ schemaVersion: 'ramify.cli/1', exitCode: 2,
      diagnostics: [{ code: 'invalid-invocation', message: expect.stringContaining('Resident service is not implemented') }] });
  });
});
