 ...
 *   `explainVisibility`'s clause and `via` must agree;
 ...
 *   `explainVisibility`'s `via` must name that module, and its clause must be
 ...
 * ancestor, since that is the one `explainVisibility` reports.
 *
  ...
  const check = (dot: DecisionDot): void => {
    ...
    for (const ref of decision.symbols) {
      if (decision.kind === 'to-parent') {
        ...
        const verdict = explainVisibility(tree, parent, ref.owner, ref.name);
      ...
      for (const reached of descendantsOf(tree, decision.decider)) {
        const verdict = explainVisibility(tree, reached, ref.owner, ref.name);
        if (!verdict.allowed) {
          fail(
...
export function validateNodeRows(tree: ModuleTree, nodes: readonly NodeLayout[]): void {
  ...
  for (const node of nodes) {
    for (const row of node.rows) {
      const verdict = explainVisibility(tree, node.id, row.owner, row.symbol);
      if (!verdict.allowed) {
        fail(
