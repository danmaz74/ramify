...
| ID | Mode / authority | Witness and meaningful expectation |
| --- | --- | --- |
| E01 | B + M / A | Required parent, sibling-through-ancestor and deeper-descendant paths work. An unexposed TypeScript export remains private across owners; actual same-owner imports remain allowed after the testing-origin guard. |
| E02 | M / A | Remove one to-parent adapter/fixture hop. The root loses the original even if its tags match. Restore that hop without changing the source import and access returns. |
...
| E06 | M / A | Add an unselected export to a file exposed only through named selections: the public Ramify contract is unchanged. A child wildcard relays the child's to-parent contract, not every export in its files. Missing child exposure and wildcard-name collisions remain diagnosed. |
...
| O05 | B / A + D | Root protocol/assembly tests use the fixed `[testing, dispatch]` profile. Workspace UI/protocol integration tests use its fixed `[testing, ui, dispatch]` profile and views already exposed to that owner. Root exposes newly defined testing-only setup and needed types to preserve actual router/client identity. Feature production peers do not gain access to each other's views or testing setup. |
...
| D04 | B + M / A + D | Native factories are exposed to parents; only selected neutral protocol contracts are exposed to descendants. Delete a hop or introduce a direct core import of dispatch plumbing to exercise distinct denials. |
...
| R01 | B + M / A | The controller consumes task helpers through the review runtime's local to-descendants path. Tasks cannot import the controller, and validation outside that subtree cannot import runtime behavior or types. |
...
| S02 | B + M / A | Namespace member, destructuring, literal lazy selection and explicit source-star forwarding follow the specified bounded selection policy. Unknown escaping/computed member use is a visible limitation, not a reason to accept every export. |
...
| P03 | Runtime grouping versus independently placed controller/tasks/validation with custom roles. Preserve both positive and negative edges; do not introduce an exact-child exposure as an assumed feature. |
