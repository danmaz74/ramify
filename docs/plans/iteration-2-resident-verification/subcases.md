# Plan 2 executable instance inventory draft

**Status:** proposed review input for iteration 2; no instance has executed.
...
synchronized check asserts `freshness.verified`, `captureStarted >=
acknowledged` and the expected `reusedRevision`. Every unit and quick row
...
| I2-02:expect-match | 6 | DA02 | daemon-service | Q/R | quick | As above with `expect` naming W's description and its new sha256. | `reported`, `verified: true`, `reusedRevision: false`. |
...
| I2-03:two-worktrees | 5 | DA03 | daemon-service | Q/R×2 | quick | Two reference copies; apply `remove-hop` in one only; open both. | Isolated contexts with different `ContextId`; one revision has the denial, the other none; fingerprints differ only in `declarations`. |
...
| I2-03:same-root-reuse | 5 | DA03 | daemon-service | Q/R | quick | Open the same root and setup twice from two leases. | Second open returns `created: false` and the same token; closing one lease leaves the context warm. |
...
| I2-05:fingerprint-classes | 4 | DA15 | contexts | M | unit | Driver reports changed description, then source, then configuration inputs. | Exactly the corresponding fingerprint changes each time; `registry` and `engine` stay constant. |
...
| I2-07:invalid-current | 4 | DA05 | contexts | M | unit | Driver returns an invalid report. | Published revision has `outcome.execution: 'invalid'`; `lastValid` remains the earlier revision. |
...
| I2-08:lost-events | 4 | DA09 | contexts | M | unit | Watcher never fires; a synchronized request arrives. | The driver is called with `cause: 'request'`; the report is published; status returns to `synchronized`. |
...
| I2-08:watcher-error | 4 | DA09 | contexts | M | unit | Watcher emits `error`. | `watcher: 'unavailable'`, `synchronization: 'watcher-unavailable'`; a synchronized request still succeeds; the manager reattaches the watcher after the next publication. |
...
| I2-09:readme-edit | 6 | DA13 | daemon-service | Q/R | quick | Edit W's README paragraph. | New revision with only `metadata` recomputed; `reused` lists the other stages; decisions unchanged. |
...
| I2-14:handshake-incompatible | 8 | DA15 | ipc | — | ipc | `hello` with `protocol: 'ramify.ipc/0'` and with a foreign `buildKey`. | `reject` naming both versions, then close; the daemon keeps serving others. |
...
| I2-17:stale-lock | 8 | PC02 | daemon-process | P | process | A lock older than 30 s with a dead pid. | Removed; startup proceeds. |
...
| I2-19:batch-no-daemon | 10 | PC01 | cli | P/R | process | `ramify check --batch` with no daemon. | No connect or spawn; `Mode: batch` in human output; the JSON document is byte-for-byte Plan 1's. |
...
| I2-23:watch-no-fallback | 9 | PC10 | cli | P/R | process | Same fault with `ramify watch`. | Exit 2 with `unavailable`; no `src/batch.js`, analysis or compiler module loaded. |
...
| I2-27:idle-disposal-releases | 12 | ML07, DA17 | lifecycle | P/R | process | `warmIdleMs: 1000`, `coldRetainMs: 1000`, `idleExitMs: 3000`. | Watcher handles, retained bytes and history reach zero; then eviction; then idle exit within `idleExitMs` plus 5 s. |
...
| I2-29:synthetic-500 | 13 | DA17 | resident-measure | P/S500 | measurement | Cold and one source edit. | Within provisional ceilings or a reviewed revision of them. |
