...
import { FocusDiagram, TreeDiagram, shopFocusDiagram, shopTreeDiagram } from "@ramify/presentation";
...
An online shop decomposed that way, four levels deep:

<TreeDiagram definition={shopTreeDiagram} />
...
- **Refinement can preserve external contracts.** A module can split its
  implementation into sub-modules and keep the same external interface by
  re-exposing the selected child contracts. Imports inside the split must
  follow the new ownership boundaries.
...
Here is the **focused view** of one module in the middle of the tree:
what reaches it from above, the module itself with its files and the
interface it exposes, to its parent or to its descendants, and what its sub-modules expose to
it. The rest of the tree is a distant map, and everything dimmed on it is not
available in `payment`, so it does not need to be read, and cannot be
imported by mistake:

<FocusDiagram definition={shopFocusDiagram} />
...
- **Show what the tree is.** Nothing declares which directories are modules.
  The tree exists in naming habits and in a few heads. You can reconstruct its
  modules, interfaces and dependencies from the code, but nothing confirms the
  reconstruction: a directory may be a module or a convenience, an export an
  interface or a leftover, an import a decision or an accident.
...
module architecture, one module's focused view, its internals plus the
interfaces it consumes, is a workload that fits, and because the tree is
declared, a tool can assemble it rather than guess it: the module's files,
plus exactly the symbols exposed to it.
...
declarations say what they are: planning at the parent means working in the
parent's focused view, where the children appear only as the interfaces they
expose; implementing means working in the child's own.
